...
module('Acceptance | meta test', function (hooks) {
  ...
  test('index meta', async function (assert) {
    ...
    assert
      ...
      .hasAttribute(
        'content',
        `Welcome Welcome to Field Guide 🎉 You have now successfully setup your own Field Guide, if things are looking a bit bare bones right now that's because Field Guide is designed to provide...`,
      );
    assert
      .dom('head link[rel="canonical"]', document)
      .hasAttribute('href', 'https://field-guide.netlify.app/');
    assert
      .dom('head meta[property="og:type"]', document)
      .hasAttribute('content', 'website');
    assert
      .dom('head meta[property="og:site_name"]', document)
      .hasAttribute('content', 'Field Guide - Field Guide');
  });
  ...
  test('index meta with a logo', async function (assert) {
    ...
    assert
      .dom('head meta[property="og:image"]', document)
      .hasAttribute(
        'content',
        'https://field-guide.netlify.app/images/face.png',
      );
  });
});
